...
export default async function PaymentsPage() {
  ...
  return (
    <>
      <PageHeader
        title="Thanh toán"
        description="Theo dõi đã thu, chưa thu và các báo cáo quá hạn."
      />
      <section className="grid-3 mb-6">
        <div className="card p-4">
          <p className="text-sm text-[var(--muted)]">Tổng cần thu</p>
          <strong className="text-2xl">{formatMoney(total)}</strong>
        </div>
        <div className="card p-4">
          <p className="text-sm text-[var(--muted)]">Đã thu</p>
          <strong className="text-2xl">{formatMoney(paid)}</strong>
        </div>
        <div className="card p-4">
          <p className="text-sm text-[var(--muted)]">Còn lại</p>
          <strong className="text-2xl">{formatMoney(total - paid)}</strong>
        </div>
      </section>
          ...
          <tbody>
            {reports.map((report) => (
              <tr key={report.id}>
                ...
                <td>
                  <StatusBadge value={report.paymentStatus} />
                </td>
                <td>
                  {report.remainingAmount > 0 ? (
                    <MarkPaidButton reportId={report.id} />
                  ) : null}
                </td>
              </tr>
            ))}
